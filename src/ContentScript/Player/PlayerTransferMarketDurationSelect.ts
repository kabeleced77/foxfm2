...
export class PlayerTransferMarketDurationSelect implements IWebElementToExtend {
  private readonly select: IHtmlSelect;
  private readonly playerTmDurationSettings: ISetting<ITransferMarketSellingDurationSettings>;
  ...
  constructor(
    ...
    this.select = select;
    this.playerTmDurationSettings = transferMarketSellingDurationSettings;
    this.log = log;
  }

  public extend(): void {
    this.log.info("start extension");
    this.playerTmDurationSettings
      .value()
      .then(setting => {
