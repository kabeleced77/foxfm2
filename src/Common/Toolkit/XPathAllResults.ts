...
export class XPathAllResults3 implements IXPathAllResults3 {
  private xPathUrl: IUrl;
  private xPathString: IXPathString;

  constructor(url: IUrl, xPath: IXPathString) {
    this.xPathUrl = url;
  ...
  public xPathAllResults(doc: Document): XPathResult {
    ...
    if (url.match(this.xPathUrl.url().toString())) {
      return doc.evaluate(this.xPath().xPathString().toString(), doc.documentElement, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
    } else {
      throw new Error(`The URL '${url}' of the given document is not the same as of the XPath: ${this.xPathUrl.url()}. Will not look for any XPath results.`);
    }
  }
  ...
  public fromJson(jsonString: String): IXPathAllResults3 {
    return new XPathAllResults3(
      this.xPathUrl.fromJson(jsonString["xPathUrl"]),
      this.xPathString.fromJson(jsonString["xPathString"])
    );
  }
}
