...
export class MessagingBackgroundScript implements IMessaging {
  ...
  public connect() {
    chrome.runtime.onConnect.addListener((port: chrome.runtime.Port) => {
      ...
      port.onMessage.addListener((message: IMessagingMessage, p: chrome.runtime.Port) => {
        ...
        switch (message.type.name) {
          case new MessagingMessageTypeAddMatchdayToIndexedDb().name:
            ...
            break;
          default:
            this.logger.error(`Unsupported messaging message type: ${message.type}`);
        }
      });
    });
  }
  ...
  private addMatchdayToIndexedDb(matchday: IMatchdayMessagingDataModel): void {
    ...
    db.transaction("rw", db.gameServers, db.matchdays, async () => {
      let gameServers = db.gameServers.filter(gs => gs.uri === hostname);
      if (await gameServers.count() === 1) {
        ...
        throw `could not add matchday to database: given game server is not supported yet: ${hostname}`;
      }
    });
  }
}
